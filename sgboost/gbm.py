...
from loss import LABEL_COLUMN, GRAD_COLUMN, HESS_COLUMN, LogisticLoss
...
class SGBClassifier(SGBModel, ClassifierMixin):
	...
	def __init__(self, loss="logisticloss", learning_rate=0.3, n_estimators=20, max_depth=6,
	...
	def predict(self, X):
		...
		# TODO: the tree prediction can be parallel
		preds = np.zeros((X.shape[0],))
		preds += self.first_round_pred
		for tree in self.trees:
			preds += self.learning_rate * tree.predict(X)
		preds = self.loss.transform(preds)
		return preds.round()